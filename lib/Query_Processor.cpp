...
__internal_loop:
	...
	ret->mirror_flagOUT=qr->mirror_flagOUT;
		...
		if (qr->ignorable==true) {
			proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5, "query rule %d has set ignorable. Will use locked hostgroup when multiplex is disabled\n", qr->rule_id);
			ret->ignorable = qr->ignorable;
		}
